...
namespace Demo.Statistics.Infrastructure.StatisticsMiddleware
{
    public class StatisticsMiddleware
    {
        ...
        {
            StatisticsAppDbContext statisticsContext = ctxReqServices.GetRequiredService<StatisticsAppDbContext>();
            if (
                    !ctxReq.IsAjaxRequest() &&
                    (ctxReq.Path.Value.Equals("/") ||
                    !ctxReq.Path.Value.StartsWith("/AdminProfile"))
            )
            {
                try
                {
                    var ctxReq = context.Request;
